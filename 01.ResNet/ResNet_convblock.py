...
class ConvLayer:
    ...
    def __init__(self, f, mi, mo, strides = 2, padding = 'VALID'):
        self.W = tf.Variable(init_filter(f, mi, mo, strides))
...
class ConvBlock:
    ...
    def __init__(self, mi, fm_sizes, strides = 2, activation = tf.nn.relu):
        ...
        self.conv_1 = ConvLayer(f = 1, mi = mi, mo = fm_sizes[0], strides)
        self.bn_1 = BNLayer(fm_sizes[0])
        # conv -> bn / f = 3, padding = 'SAME' --> output_shape = ( mi, mi, mo )
        self.conv_2 = ConvLayer(f = 3, mi = fm_sizes[0], mo = fm_sizes[1], strides = 1, padding = 'SAME')
        self.bn_2 = BNLayer(fm_sizes[1])
        # conv -> bn / f = 1 --> output_shape = ( (mi - f)/2 + 1, (mi - f)/2 + 1, mo )
        self.conv_3 = ConvLayer(f = 1, mi = fm_sizes[1], mo = fm_sizes[2], strides = 1)
        self.bn_3 = BNLayer(fm_sizes[2])

        # Skip connection
        # Conv -> bn / f = 1, s = 2 --> output_shape = ( (mi -f)/2 + 1, (mi -f)/2 + 1, mo )
        self.conv_s = ConvLayer(1, mi, fm_sizes[2], strides)
...
if __name__=='__name__':
    ...
    conv_block = ConvBlock(mi = 3, fm_sizes= [64, 64, 256], strides = 1)
