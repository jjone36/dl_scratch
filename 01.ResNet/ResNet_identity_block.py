...
class IdentityBlock:
    ...
    '''
    def __init__(self, mi, fm_sizes, strides = 2, activation):
        ...
        self.conv_2 = ConvLayer(f = 3, fm_sizes[0], fm_sizes[1], strides = 1, padding = 'SAME')
        self.bn_2 = BNLayer(fm_sizes[1])
        # ConvBLock_3 : conv -> bn / f = 3 --> output_shape = ( (mi - f)/2 + 1, (mi - f)/2 + 1, mo )
        self.con_3 = ConvLayer(f = 1, fm_sizes[1], fm_sizes[2], strides = 1)
